...
public class PlayerControl extends ListenerAdapter {
    ...
    // leave        - Leaves the voice channel that the Mage is currently in.
    ...
    {
        if (!(event.getMessage().getContent().startsWith("!music")) ||
                event.getMessage().getAuthor().isBot() ||
                !event.isFromType(ChannelType.TEXT)) return;

        if (System.getProperty("os.name").equals("Mac OS X")) {
            event.getChannel().sendMessage("Music currently doesn't work because " + Mage.settings.getAuthorAsMention() +
                    " is currently using an inferior OS. Tell him to get on the superior Windows or the fantastic Linux.").queue();
            return;
        }
        String[] command = event.getMessage().getContent().replaceFirst("!music ", "").split(" ", 2);
            ...
            else
            {
                ...
                else
                {
                    ...
                    catch (PermissionException e)
                    {
                        ...
                        {
                            event.getChannel().sendMessage("Mage does not have permission to connect to: " + chan.getName()).queue();
                        }
                    }
                }
            }
